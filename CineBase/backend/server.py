...
async def get_daily_suggestions():
    """Get daily random suggestions - changes based on date"""
    import random
    from datetime import date
    
    # Use current date as seed for consistent daily randomness
    today = date.today()
    random.seed(today.toordinal())
    
    # Get random actors (limit 6)
    total_actors = await db.actors.count_documents({})
    actor_skip = random.randint(0, max(0, total_actors - 6)) if total_actors > 6 else 0
    actors = await db.actors.find().skip(actor_skip).limit(6).to_list(6)
    
    # Shuffle actors for more randomness
    if actors:
        random.shuffle(actors)
        actors = actors[:6]
    
    # Get random movies (limit 6) 
    total_movies = await db.movies.count_documents({})
    movie_skip = random.randint(0, max(0, total_movies - 6)) if total_movies > 6 else 0
    movies = await db.movies.find().skip(movie_skip).limit(6).to_list(6)
    
    # Shuffle movies for more randomness
    if movies:
